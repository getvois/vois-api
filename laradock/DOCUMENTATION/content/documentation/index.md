...
*App: enter MySQL container*

```bash
docker-compose exec mysql bash
```

*App: enter to MySQL prompt within MySQL container*
...
1 - Run the NetData Container (`netdata`) with the `docker-compose up` command. App:
...
1 - Run the Metabase Container (`metbase`) with the `docker-compose up` command. App:
...
3. Then you have to add new config section into docker-compose.yml with related variables:
    ...
    custom_proxy_name:
          ...
          networks:
            - frontend
    ``` 
4. change your varnish config and add nginx configuration. App Nginx configuration is here: `nginx/sites/laravel_varnish.conf.example`.
...
1 - Run the phpMyAdmin Container (`phpmyadmin`) with the `docker-compose up` command. App:
...
1 - Run the Gitlab Container (`gitlab`) with the `docker-compose up` command. App:
...
4 - Run the Gitlab-Runner Container (`gitlab-runner`) with the `docker-compose up` command. App:
...
1 - Run the Adminer Container (`adminer`) with the `docker-compose up` command. App:
...
1 - Run the Portainer Container (`portainer`) with the `docker-compose up` command. App:
...
1 - Run the pgAdmin Container (`pgadmin`) with the `docker-compose up` command. App:
...
1 - Run the Confluence Container (`confluence`) with the `docker-compose up` command. App:
...
1 - Run the Selenium Container (`selenium`) with the `docker-compose up` command. App:
...
2 - Run the Minio Container (`minio`) with the `docker-compose up` command. App:
...
2 - Run the Thumbor Container (`minio`) with the `docker-compose up` command. App:
...
2 - Run the Aws Container (`aws`) with the `docker-compose up` command. App:
...
That's it! Run your containers.. App:
